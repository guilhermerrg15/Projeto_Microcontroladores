...
void loop() {
  ...
  if (idx_tela == Tela_2_1_3) {
   ...
   if (idx_tela == Tela_2_2) {
    if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
      rfidHex = "";
      myFile = SD.open("guilherme.txt", O_CREAT | O_WRITE);
      if (myFile) {
...
void tela_2_2_3() {
  ...
  for(int i = 0; i <26; i++){
    ...
    listaBotoes[i].init(&tela,&touch,x,y,40,40,TFT_BLACK,TFT_WHITE,TFT_RED,listaLetras[i],2);
...
void tela_2_2_2() {
  ...
  botao_Renomeia_ID.setPressHandler(goto_tela_2_2_3);
}
