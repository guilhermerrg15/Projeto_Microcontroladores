...
void loop() {
  ...
  if (idx_tela == Tela_Inicial) {
    showBMP("logo.bmp", 55, 25); //colocar logo no sd card
...
void tela_1() {
  tela.fillScreen(TFT_BLACK);
  botao_RFID.init(&tela, &touch, 5, 260, 100, 110, TFT_BLACK, TFT_RED,
                  TFT_WHITE, "RFID", 2);
  botao_Infra.init(&tela, &touch, 180, 260, 100, 110, TFT_BLACK, TFT_RED,
                   TFT_WHITE, "Infravermelho", 2);
...
void tela_inicial() {
    tela.fillScreen(TFT_BLACK);
    botao_Iniciar.init(&tela, &touch, 200, 300, 100, 80, TFT_BLACK, TFT_RED, TFT_WHITE, "Iniciar", 2);
